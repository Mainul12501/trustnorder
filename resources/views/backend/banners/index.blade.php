            ...
            <div class="col-12">
                <div class="card card-hover">
                    <div class="card-header bg-info">
                        <h4 class="text-white float-start">Banners</h4>
{{--                        @can('create-permission-category')--}}
                            <a href="{{  route('banners.create') }}" class="rounded-circle float-end text-white text-light f-s-20 ">
                            ...
                            <table class="table responsive dt-responsive table-responsive"  id="dataTable">
                                ...
                                @foreach($banners as $key => $banner)
                                    <tr>
                                        ...
                                        <td>{!! str()->words(strip_tags($banner->description), 30) ?? '' !!}</td>

                                        <td>{{ $banner->status == 1 ? 'Published' : 'Unpublished' }}</td>
                                        <td class="">
                                            <a href="{{ route('banners.show', $banner->id) }}" class="btn btn-sm btn-primary mt-1">
                                                <i class="mdi mdi-eye"></i>
                                            </a> <br>
                                            <a href="{{ route('banners.edit', $banner->id ) }}" class="btn btn-sm btn-warning mt-1">
